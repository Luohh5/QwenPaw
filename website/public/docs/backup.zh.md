...
| 模块             | 物理路径                            | 实际内容                                                                                                                             |
| ---------------- | ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| **智能体工作区** | `~/.qwenpaw/workspaces/<agent_id>/` | 每个智能体工作目录下的全部文件，例如人设文件、记忆、技能、聊天历史、频道配置，以及邮箱配置和本地索引（可能含频道凭据与邮箱授权码）。 |
...
- 备份文件**可能包含敏感凭证**：完整备份默认会打包密钥信息（模型 API Key、加密主密钥、控制台登录凭证等）；即使是部分备份，只要包含智能体工作区，也可能包含频道凭据（`bot_token`、`app_secret` 等）和 `agent.json` / `drivers/mcp/qwenpawmail.yaml` 中的邮箱授权码。**请妥善保管备份文件，不要分享给他人。**
